...
async fn send_batch(
    ...
    headers.insert(
        ...
        HeaderValue::from_str(service_role_key).map_err(|err| StackError::SupabaseSinkHttp {
            status: 0,
            body: format!("invalid service role key header: {err}"),
        })?,
    );
    headers.insert(
        reqwest::header::AUTHORIZATION,
        HeaderValue::from_str(&bearer).map_err(|err| StackError::SupabaseSinkHttp {
            status: 0,
            body: format!("invalid authorization header: {err}"),
        })?,
    );
    headers.insert(
