//! Hand-curated catalog of trusted Agent Skills sources.
//!
//! The embedded `data/skills.toml` records official skill directories that
//! `acp-stack` may later use for opt-in installation or provisioning. Loading
//! this catalog is intentionally side-effect free: it does not fetch, install,
//! or modify agent-owned config.
...
const OFFICIAL_SOURCES: &[(&str, &str)] = &[("anthropics", "skills"), ("openai", "skills")];
...
impl SkillSource {
    fn validate(&self) -> Result<()> {
        ...
        if let Some(commit) = self.verified_commit.as_deref() {
            validate_commit_sha(&self.id, commit)?;
        }
        ...
        if self.directories.is_empty() {
            return Err(StackError::RegistryLoad {
                reason: format!(
                    "skill source `{}` must declare at least one directory",
                    self.id
                ),
            });
        }
...
fn validate_commit_sha(source_id: &str, commit: &str) -> Result<()> {
    if commit.len() == 40 && commit.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Ok(());
    }
    Err(StackError::RegistryLoad {
        reason: format!(
            "skill source `{source_id}` verified_commit must be a full 40-character hex SHA"
        ),
    })
}
