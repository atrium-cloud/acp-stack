//! Runtime-identity rules (`runtime.user_mismatch`, `runtime.workspace_not_writable`), both turning on the daemon's effective uid versus the configured workspace setup.
...
pub(in crate::security) fn check_runtime_user(
    ...
    if let Some(uid) = inputs.runtime_user_uid {
        if uid != inputs.process_euid {
            // Root execution is dev-only, so remediation never tells an operator to relaunch the production daemon as root.
    ...
    if !inputs.workspace_writable {
        findings.push(
            ...
            // The probe runs as the daemon's effective uid, so the hint must reference that uid,
            // not `runtime_user_name`, which can resolve to a different one.
            .with_remediation(format!(
