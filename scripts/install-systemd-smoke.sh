...
readonly CONTAINER_NAME="acp-stack-systemd-smoke-$$"
readonly IMAGE="${ACP_STACK_SYSTEMD_SMOKE_IMAGE:-ubuntu:24.04}"
readonly HOST_PORT="${ACP_STACK_SYSTEMD_SMOKE_PORT:-17700}"
readonly STATUS_URL="http://127.0.0.1:${HOST_PORT}/v1/status"
readonly RUNTIME_USER="${ACP_STACK_SYSTEMD_SMOKE_USER:-acp_smoke}"
readonly HOME_DIR="/home/${RUNTIME_USER}"
readonly WORKSPACE_ROOT="${ACP_STACK_SYSTEMD_SMOKE_WORKSPACE:-/srv/acp-stack}"
...
usage() {
  cat <<'USAGE'
Usage: scripts/install-systemd-smoke.sh

Builds the acps + acpctl binaries (or reuses an existing release build when
ACP_STACK_SKIP_BUILD=1), starts a privileged ubuntu:24.04 container running
/sbin/init, runs scripts/install-systemd.sh inside it, enables the unit,
waits for it to become active, and probes GET /v1/status using the session
key parsed from acps init output.

Env knobs:
  ACP_STACK_SKIP_BUILD=1         Skip cargo build (binaries must already exist
                                 at target/release/acps and acpctl)
  ACP_STACK_SYSTEMD_SMOKE_PORT   Host port mapped to container 7700 (default 17700)
  ACP_STACK_SYSTEMD_SMOKE_IMAGE  Base image (default ubuntu:24.04)
USAGE
}
...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "install-systemd-smoke: refusing to run on non-Linux host (got $(uname -s))." >&2
...
if ! command -v docker >/dev/null 2>&1; then
  echo "install-systemd-smoke: docker is required." >&2
  exit 1
fi

if ! command -v curl >/dev/null 2>&1; then
  echo "install-systemd-smoke: curl is required." >&2
  exit 1
fi
...
if [[ "${ACP_STACK_SKIP_BUILD:-0}" != "1" ]]; then
  echo "install-systemd-smoke: cargo build --release --bin acps --bin acpctl..."
  (cd "${REPO_ROOT}" && cargo build --release --bin acps --bin acpctl)
fi

if [[ ! -x "${acps_binary}" || ! -x "${acpctl_binary}" ]]; then
  echo "install-systemd-smoke: built binaries missing at ${acps_binary} or ${acpctl_binary}." >&2
  exit 1
fi

echo "install-systemd-smoke: starting systemd container ${CONTAINER_NAME}..."
docker run -d \
  --name "${CONTAINER_NAME}" \
  --privileged \
  --cgroupns=host \
...
echo "install-systemd-smoke: waiting for systemd to settle..."
...
echo "install-systemd-smoke: bootstrapping container packages (systemd-sysv, ca-certificates)..."
docker exec -e DEBIAN_FRONTEND=noninteractive "${CONTAINER_NAME}" \
  bash -c 'apt-get update -qq && apt-get install -y -qq --no-install-recommends systemd-sysv ca-certificates' \
  >/dev/null

echo "install-systemd-smoke: running install-systemd.sh inside container..."
...
if [[ -z "${session_key}" ]]; then
  echo "install-systemd-smoke: failed to parse session key from installer output." >&2
  exit 1
fi

echo "install-systemd-smoke: asserting config and unit reflect installer options..."
...
echo "install-systemd-smoke: verifying idempotent installer re-run..."
docker exec "${CONTAINER_NAME}" \
  bash /src/scripts/install-systemd.sh \
    --acps-binary /src/target/release/acps \
    --acpctl-binary /src/target/release/acpctl \
...
echo "install-systemd-smoke: verifying --force cannot drift config-managed paths..."
if docker exec "${CONTAINER_NAME}" \
  bash /src/scripts/install-systemd.sh \
    ...
    --no-os-deps \
    --force; then
  echo "install-systemd-smoke: installer unexpectedly allowed --force to drift workspace config." >&2
  exit 1
fi

echo "install-systemd-smoke: verifying unit drift requires --force..."
...
if docker exec "${CONTAINER_NAME}" \
  bash /src/scripts/install-systemd.sh \
    ...
    --bind 0.0.0.0:7700 \
    --no-os-deps; then
  echo "install-systemd-smoke: installer unexpectedly accepted a drifted unit without --force." >&2
  exit 1
fi
docker exec "${CONTAINER_NAME}" \
  bash /src/scripts/install-systemd.sh \
...
echo "install-systemd-smoke: enabling and starting acp-stack.service..."
docker exec "${CONTAINER_NAME}" systemctl enable --now acp-stack.service >/dev/null

echo "install-systemd-smoke: waiting for acp-stack to become active..."
active=false
for _ in $(seq 1 30); do
...
if [[ "${active}" != true ]]; then
  echo "install-systemd-smoke: acp-stack failed to become active." >&2
...
echo "install-systemd-smoke: probing ${STATUS_URL}..."
status_body="$(mktemp)"
...
if [[ "${status_ok}" != true ]] || ! grep -Eq '"ok"[[:space:]]*:[[:space:]]*true' "${status_body}"; then
  echo "install-systemd-smoke: /v1/status check failed." >&2
  docker exec "${CONTAINER_NAME}" systemctl status acp-stack.service --no-pager >&2 || true
  docker exec "${CONTAINER_NAME}" journalctl -u acp-stack.service --no-pager -n 200 >&2 || true
  exit 1
fi

echo "install-systemd-smoke: PASS"
