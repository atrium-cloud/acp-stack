...
- `reqwest` with rustls and blocking support - CLI HTTP client for daemon-backed commands such as `acps agent start` and `acps agent stop`, HTTPS `public_url` targets, synchronous ACP registry fetches during agent installation, and the async Supabase logging sink that POSTs batched rows to PostgREST with `Prefer: resolution=merge-duplicates`.
