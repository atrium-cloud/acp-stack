...
- `sha2` - SHA-256 hashing of installed agent binaries for the optional `expected_sha256` integrity check; also used by `runtime::install::github_release` to verify a downloaded release asset against a sibling `checksums.txt` when the registry entry declares one, and by `runtime::workspace_sources::s3_client` for SigV4 payload hashing.
- `hmac` (pinned to 0.12 so it shares `digest` 0.10 with `sha2`) - HMAC-SHA256 used by `runtime::workspace_sources::s3_client` to derive the SigV4 signing key chain (`date_key`, `region_key`, `service_key`, `signing_key`).
- `roxmltree` - minimal XML parser used by `runtime::workspace_sources::s3_client` to decode `ListObjectsV2` responses. Avoids pulling the full AWS SDK for the two S3 operations the workspace materializer needs.
...
- `tokio-rustls` (dev-dependency) - local HTTPS test server for the `acps init --data-from https://...` archive-ingestion integration test. Keeps production CLI validation strict while avoiding external network fixtures.
- `base64` (dev-dependency) - decodes the embedded localhost TLS certificate/key fixtures used by the HTTPS archive-ingestion integration test.
