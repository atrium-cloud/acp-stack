...
- Request: starts one active init session. Accepts optional initial agent/provider/model/workspace args, environment configuration declarations, and an in-memory native config upload. Unknown request fields are rejected. Body fields, grouped:
    ...
    - Agent CLI version and existing installs (mirror the `--agent-version`/`--existing-agent` flags; declared up-front, never streamed):
        - `agent_version`: the agent CLI version to install instead of its latest release. A GitHub install fetches it as its release tag verbatim; an npm install uses it as the package version with one leading `v` dropped when a digit follows it. Letters, digits, `.`, `_`, `+`, and `-`, starting with a letter or digit. A CLI installed only by its vendor's script, bundled inside its adapter, or installed by `[agent.install]` fails the session with `agent.version_unsupported`.
        - `existing_agent` (`use-existing` | `replace-latest` | `replace-version`): what init does with an agent CLI acp-stack did not install. `replace-latest` also clears `[agent].harness_version`. `replace-version` requires `agent_version`; `use-existing` and `replace-latest` conflict with it; each violation is a `400`.
        - Absent `existing_agent` replaces such a CLI with `agent_version` when given, else with the configured `[agent].harness_version`, else with its latest release. An ACP adapter acp-stack did not install is always replaced with its latest release.
