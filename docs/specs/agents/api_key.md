...
- The remaining endpoint-carrying configurations (custom providers, and mapped providers without such a variable) are provisioned as a managed named `providers.acps-managed` entry with `key_env` and `transport`, referenced as `model.provider: custom:acps-managed` and never as `model.base_url`.
