...
- The run removes any artifact it produced from the session cwd afterward, verified or not, so the workspace is left as it was found rather than carrying a probe file into later sessions. A failed removal is logged and leaves the verdict unchanged.
