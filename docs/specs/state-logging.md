...
Each session row records `target_id` (the Array target that owns the session) and `agent_session_id` (the session id the agent assigned), alongside the local session `id`. A `UNIQUE(target_id, agent_session_id)` index makes the agent's session id the stable per-target identity, so the same agent session id may recur under different targets; an index on `(target_id, updated_at DESC, id DESC)` backs target-scoped listing. A legacy single-agent database backfills `target_id` from the session's `agent_id` and `agent_session_id` from the local `id`. See [array.md](array.md).

## Auth Keys Table

`auth_keys` stores one verifier row for each auth key kind: `session` and `admin`. Rows contain the verifier algorithm, salt, digest, and timestamps. Plaintext auth keys are never stored in SQLite, config, or `secrets.age`.
...
The setup SQL is pushed as a versioned Supabase CLI migration, and the whole script is idempotent (create-if-missing tables, additive column guards for later schema additions, create-or-replace functions and views). When the schema shape changes, a new migration version carries the full script and superseded versions are kept as placeholders, so re-running setup upgrades an existing mirror in place without dropping mirrored data. Re-running against a mirror already at the current version applies nothing: the Supabase CLI skips migration versions it has recorded. Setup therefore reuses the writer password already stored in the secret store — a fresh one is generated only on first setup, or when the stored URL is a custom shape the runtime cannot round-trip — so the stored connection URL and the remote role stay consistent whether or not the migration applies.
