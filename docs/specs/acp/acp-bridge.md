...
Discovery probes follow the same sequencing. The provisional session created for `GET /v1/models`, `GET /v1/agent/config-options`, and the CLI effort validation applies the requested or configured model through `session/set_config_option` before its options are read, so the per-model values (reasoning effort above all) describe that model rather than the one the harness booted with. The model set and `session/new` share one timeout budget. The set is skipped for a harness that reads its model pin from on-disk config and for a model the session already sits on; an unadvertised value or a rejected set leaves the `session/new` response as the answer and is logged.

Both option sets stay available to the caller. Post-set options are overlaid onto the `session/new` set by option id, so an adapter answering with only the options its change touched keeps the rest of the advertisement. Model and mode values, which the agent offers independently of the applied model, are read from `session/new` itself; reasoning-effort levels are read from the overlay. Paths that validate or list models rather than describe one, such as `POST /v1/agent/switch`, the native-config import, and model resolution in `acps agent set`, apply no model at all.
