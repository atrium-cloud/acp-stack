...
Session create applies the same routing to `agent.mode`, the configured model, and `agent.effort` (the agent's `thought_level` config option, applied after the model since adapters advertise effort levels per model): when the agent's `session/new` config options do not advertise the configured value, the set is skipped, the session proceeds on the agent's default, and a `session.capability_ignored` event (level `warn`) records the omission. A failure from setting an option the agent did advertise remains a hard error.
