...
Session create applies the same routing in a fixed order: `agent.mode`, then the configured model, then `agent.effort`, and finally every `[agent.config_options]` entry. `agent.effort` maps to the agent's `thought_level` config option and applies after the model, since adapters advertise effort levels per model.
