...
acps init \
  ...
  [--skills-source <openai|anthropic|github:owner>] [--skills <name,name>] \
  [--plugins-source <openai>] [--plugins <name,name>] [--no-skills] \
...
Official skill sources are `openai` and `anthropic`; custom sources use
`github:<owner>` and expect `<owner>/skills` on branch `main`.
`--plugins-source openai` and `--plugins` install OpenAI plugin bundles as their
contained skills.
