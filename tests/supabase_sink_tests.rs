...
fn supabase_config(url: &str) -> SupabaseLoggingConfig {
    SupabaseLoggingConfig {
        enabled: true,
        url: url.to_owned(),
        service_role_key_ref: "SUPABASE_SERVICE_ROLE_KEY".to_owned(),
...
async fn happy_path_uploads_grouped_batches_and_marks_sent() {
    ...
    let sink = SupabaseSink::spawn(
        state.clone(),
        supabase_config(&url),
        "test-service-role-key".to_owned(),
        event_hub,
    )
    .expect("sink spawn");
    ...
    for req in &captured {
        ...
        assert_eq!(apikey, "test-service-role-key");
        let auth = req
            .header("Authorization")
            .expect("authorization header present");
        assert_eq!(auth, "Bearer test-service-role-key");
    }
